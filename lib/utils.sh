...
# ======= Rclone wrapper function =======

# Wrapper function for rclone that formats NOTICE messages with [WARNING] label
# This function intercepts rclone output and reformats specific messages for better integration
# with the backup system's logging format.
#
# Usage: rclone_with_labels [rclone arguments...]
# Example: rclone_with_labels listremotes
# Example: rclone_with_labels copy source dest --progress
#
# Features:
# - Reformats NOTICE messages to include [WARNING] label for consistency
# - Preserves all rclone functionality and exit codes
# - Maintains real-time output for progress indicators
# - Handles both stdout and stderr streams appropriately
rclone_with_labels() {
    ...
    # Execute rclone with output processing
    # Use process substitution to handle both stdout and stderr
    {
        # Capture both stdout and stderr, but process them differently
        rclone "$@" 2>&1 | while IFS= read -r line; do
            # Check if the line contains NOTICE messages that should be reformatted
            if [[ "$line" =~ ^[0-9]{4}/[0-9]{2}/[0-9]{2}\ [0-9]{2}:[0-9]{2}:[0-9]{2}\ NOTICE: ]]; then
                # Extract the timestamp part (first 19 characters: YYYY/MM/DD HH:MM:SS)
                local timestamp="${line:0:19}"
                # Extract the message part (everything after "YYYY/MM/DD HH:MM:SS NOTICE: ")
                local message="${line:28}"
                # Reformat with [WARNING] label
                echo "${timestamp} [WARNING] NOTICE: ${message}"
            elif [[ "$line" =~ ^[0-9]{4}/[0-9]{2}/[0-9]{2}\ [0-9]{2}:[0-9]{2}:[0-9]{2}\ ERROR: ]]; then
                # Extract the timestamp part
                local timestamp="${line:0:19}"
                # Extract the message part (everything after "YYYY/MM/DD HH:MM:SS ERROR: ")
                local message="${line:26}"
                # Reformat with [ERROR] label
                echo "${timestamp} [ERROR] ERROR: ${message}"
            elif [[ "$line" =~ ^[0-9]{4}/[0-9]{2}/[0-9]{2}\ [0-9]{2}:[0-9]{2}:[0-9]{2}\ INFO: ]]; then
                # Extract the timestamp part
                local timestamp="${line:0:19}"
                # Extract the message part (everything after "YYYY/MM/DD HH:MM:SS INFO: ")
                local message="${line:25}"
                # Reformat with [INFO] label  
                echo "${timestamp} [INFO] INFO: ${message}"
            else
                # For all other lines (progress, stats, etc.), pass through unchanged
                echo "$line"
            fi
        done
        # Return the exit code of rclone, not the while loop
    } || return $?
}

# Alias function for backward compatibility and convenience
# This allows existing code to be easily updated to use the labeled version
rclone_labeled() {
    rclone_with_labels "$@"
}

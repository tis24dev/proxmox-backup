...
setup_configuration() {
    ...
    if [[ -f "env/backup.env.example" ]]; then
        ...
        print_warning "Example configuration file not found, creating basic config"
        cat > env/backup.env << 'EOF'
#!/bin/bash
# Basic Proxmox Backup System Configuration
SCRIPT_VERSION="1.0.0"
...
LOCAL_BACKUP_PATH="/opt/proxmox-backup/backup/"
LOCAL_LOG_PATH="/opt/proxmox-backup/log/"
...
setup_cron() {
    ...
    (crontab -l 2>/dev/null; echo "0 2 * * * $INSTALL_DIR/script/proxmox-backup.sh") | crontab -
...
run_first_backup() {
    ...
    if ./script/proxmox-backup.sh --dry-run; then
        ...
        print_warning "Check the logs for more information"
    fi
}
